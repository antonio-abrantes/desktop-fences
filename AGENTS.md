...
App nativo Windows 11 (C# / .NET 8 / WPF) que agrupa ícones reais da área de trabalho em “fences” translúcidas, no espírito do Stardock Fences. As Fases 1–6 e o hotfix `v0.5.1` estão fechados. A Fase 7 entrega na `v0.6.0` instaladores por usuário x64/ARM64, upgrade no lugar, idioma inicial, desinstalação segura e reconciliação conservadora de itens apagados enquanto o app esteve fechado; implementação automática concluída, com gate manual pendente. O hotfix `v0.6.3` acrescenta arranque multi-monitor, layout padrão de novas fences, flicker em sobreposição e remoção de fence com confirmação + barreira de devolução. A `v0.6.4` acrescenta **Nova fence** no menu de contexto do desktop (instalação). A `v0.6.5` corrige o skip de z-order (fences sobrepostas em idle), ignora inbound ao mover uma fence, e ajusta a grelha das Configurações. A spec de upgrade seguro do instalador está escrita e **não** entra nesta tag. Duplo clique no desktop e packs de tema estão fora deste ciclo.
...
1. A etapa vigente está em `docs/SESSION-HEADER.md` e o detalhe operacional em `docs/plano-implementacao.md`. As Fases 1–6 estão fechadas. A Fase 7 está implementada e validada automaticamente; o gate humano de instalação/upgrade/desinstalação permanece aberto. A `v0.6.5` fecha o polish de flicker/Settings; o hotfix do instalador (`spec-hotfix-instalador-upgrade-seguro.md`) **não** começa sem pedido. Duplo clique no desktop, packs de tema e empurrar vizinhos **não** entram neste ciclo.
