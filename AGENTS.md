...
App nativo Windows 11 (C# / .NET 8 / WPF) que agrupa ícones reais da área de trabalho em “fences” translúcidas, no espírito do Stardock Fences. O **MVP 2** (`v0.3.1`) entrega N fences, Configurações (cores, idioma pt/en, iniciar com o Windows) e o hide/restore do MVP 1. A Fase 3 (arrastar item entre fences) está **fechada**. A seguinte (snap) só com autorização no `SESSION-HEADER.md` e pedido explícito.
...
1. A etapa vigente está em `docs/SESSION-HEADER.md` e o detalhe operacional em `docs/plano-implementacao.md`. O MVP 1 e o MVP 2 (Fases 1–2) estão fechados. A Fase 3 está fechada. O ciclo restante (Fases 4–7) está no plano; o mapa curto é `docs/pos-mvp1.md`. Empurrar vizinhos ao expandir **não** entra neste ciclo.
